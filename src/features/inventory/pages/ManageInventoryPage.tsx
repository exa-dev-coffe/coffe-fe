...
import { useMenusQuery, useUpdateMenuAvailabilityMutation } from "@/features/menu/hooks/useMenu.ts";
...
import { HiOutlineCube, HiOutlineSearch, HiOutlineSwitchHorizontal, HiStar } from "react-icons/hi";
...
export const ManageInventoryPage: React.FC = () => {
    ...
    const { data: menuResponse, isLoading: loading } = useMenusQuery(page, 12, debouncedSearch);
    const { mutateAsync: updateAvailableMenu, isPending: actionLoading } = useUpdateMenuAvailabilityMutation();

    const data = menuResponse?.data || [];
    const totalData = menuResponse?.totalData || 0;

    const [modalState, setModalState] = useState<{ open: boolean; id: number | null; currentAvailable: boolean }>({
        open: false,
        id: null,
    ...
    return (
        <div className="space-y-6">
            ...
            {loading && data.length === 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {Array.from({ length: 8 }).map((_, i) => (
                        <div key={i} className="p-4 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 space-y-3">
                            ...
                            >
                                <div className="relative aspect-[16/10] w-full overflow-hidden bg-slate-100 dark:bg-slate-800">
                                    ...
                                    <div className="absolute top-3 left-3">
                                        <Badge variant={item.isAvailable ? "success" : "danger"} size="sm" dot>
            ...
            <Modal
                ...
                handleClose={() => setModalState({ open: false, id: null, currentAvailable: true })}
                size="sm"
