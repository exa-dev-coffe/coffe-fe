...
import { TableFormSchema, type TableFormData, type TableItem } from "@/features/tables/types/table.types.ts";

export const useTableQuery = (page: number, pageSize: number, search?: string) => {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            try {
                ...
                const res = await fetchWithRetry<BaseResponse<PaginationData<TableItem[]>>>({
                    url,
                    method: "get",
                });
                ...
                return { data: [], totalData: 0, totalPages: 0 } as unknown as PaginationData<TableItem[]>;
            } catch (err) {
                handleApiError(err, "Failed to load tables", errorNotificationDashboard);
                throw err;
            }
        }
    });
        ...
        queryFn: async () => {
            try {
                ...
                if (res?.data?.success && Array.isArray(res.data.data)) {
                    return res.data.data.map((t) => ({ value: Number(t.id), label: t.name }));
                }
                return [];
            } catch (err) {
                handleApiError(err, "Failed to load tables", errorNotificationDashboard);
...
export const useAddTableMutation = () => {
    ...
    return useMutation({
        ...
        onError: (err: Error) => {
            if (!(err instanceof ZodError) && !err?.name) { // simplistic check for zod formatted errors vs normal errors
                handleApiError(err, "Failed to create table", errorNotificationDashboard);
            }
        }
    });
};
    ...
    return useMutation({
        ...
        onError: (err: Error) => {
            if (!(err instanceof ZodError) && !err?.name) {
                handleApiError(err, "Failed to update table", errorNotificationDashboard);
            }
        }
    });
};
