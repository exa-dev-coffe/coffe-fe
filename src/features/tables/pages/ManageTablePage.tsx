...
import { HiOutlineViewBoards, HiOutlinePlus, HiOutlineSearch, HiX } from "react-icons/hi";
...
export const ManageTablePage: React.FC = () => {
    ...
    const { data: tableData, isLoading: queryLoading } = useTableQuery(page, 10, debouncedSearch);
    const { mutateAsync: addTable, isPending: addLoading, error: addError } = useAddTableMutation();
    const { mutateAsync: updateTable, isPending: updateLoading, error: updateError } = useUpdateTableMutation();
    const { mutateAsync: deleteTable, isPending: deleteLoading } = useDeleteTableMutation();

    const data = tableData?.data || [];
    const totalData = tableData?.totalData || 0;

    const [drawerState, setDrawerState] = useState<{ open: boolean; mode: "add" | "edit"; id: number | null }>({
        open: false,
        mode: "add",
        id: null,
    });
    const [nameInput, setNameInput] = useState("");
    const [deleteModalState, setDeleteModalState] = useState<{ open: boolean; id: number | null }>({
        open: false,
        id: null,
    });
    ...
    const getFormError = () => {
        ...
        if (drawerState.mode === "edit" && updateError) {
            const updateErrors = (updateError as unknown as Record<string, string>) || {};
            return updateErrors.name || undefined;
        }
        return undefined;
    };
                ...
                <Card variant="dashboard" className="border-amber-500/30 bg-amber-500/5 animate-fade-in">
                    <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800">
                        <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">
                            {drawerState.mode === "add" ? "Create New Seating Table" : "Edit Seating Table"}
                        </h3>
                        <button
            ...
            {queryLoading && data.length === 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {Array.from({ length: 6 }).map((_, i) => (
                        <div key={i} className="p-5 rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 space-y-3">
