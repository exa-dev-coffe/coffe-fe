...
export const useBaristaQuery = (page: number, pageSize: number, search?: string) => {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            try {
                ...
                const res = await fetchWithRetry<BaseResponse<PaginationData<BaristaItem[]>>>({
                    url,
                    method: "get",
                });
                ...
                return { data: [], totalData: 0, totalPages: 0 } as unknown as PaginationData<BaristaItem[]>;
            } catch (err) {
                handleApiError(err, "Failed to load baristas", errorNotificationDashboard);
                throw err;
            }
        }
    });
    ...
    return useMutation({
        ...
        onError: (err: Error | ZodError) => {
            if (!(err instanceof ZodError) && !err?.name) {
                handleApiError(err, "Failed to register barista", errorNotificationDashboard);
            }
        }
    });
};
...
export const useDeleteBaristaMutation = () => {
    ...
    return useMutation({
        ...
        onError: (err) => {
            handleApiError(err, "Failed to remove barista", errorNotificationDashboard);
        }
    });
};
