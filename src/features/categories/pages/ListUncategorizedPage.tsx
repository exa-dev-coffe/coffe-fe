...
export const ListUncategorizedPage: React.FC = () => {
    ...
    const { data: uncategorizedResponse, isLoading: loading } = useUncategorizedMenuQuery(page, 10);
    ...
    const optionsCategory = categories.map((c) => ({
        value: Number(c.id),
        label: c.name,
    }));
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                        Assign each product to its respective category so it appears correctly in customer filters.
                    </p>
                </div>
            </div>
