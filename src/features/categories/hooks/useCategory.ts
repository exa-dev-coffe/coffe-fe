...
import { CategoryFormSchema, type CategoryFormData, type CategoryItem, type UncategorizedMenuItem } from "@/features/categories/types/category.types.ts";

export const useCategoryQuery = (page: number, pageSize: number, search?: string) => {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            try {
                ...
                const res = await fetchWithRetry<BaseResponse<PaginationData<CategoryItem[]>>>({
                    url,
                    method: "get",
                });
                ...
                return { data: [], totalData: 0, totalPages: 0 } as unknown as PaginationData<CategoryItem[]>;
            } catch (err) {
                handleApiError(err, "Failed to load categories", errorNotificationDashboard);
                throw err;
            }
        }
    });
    ...
    return useMutation({
        ...
        onError: (err: Error) => {
            if (!(err instanceof ZodError) && !err?.name) {
                handleApiError(err, "Failed to create category", errorNotificationDashboard);
            }
        }
    });
};
...
export const useDeleteCategoryMutation = () => {
    ...
    return useMutation({
        ...
        onError: (err) => {
            handleApiError(err, "Failed to delete category", errorNotificationDashboard);
        }
    });
};
    ...
    return useQuery({
        ...
        queryFn: async () => {
            try {
                ...
                const res = await fetchWithRetry<BaseResponse<PaginationData<UncategorizedMenuItem[]>>>({
                    url,
                    method: "get",
                });
                if (res?.data?.success && res.data.data) {
                ...
                return { data: [], totalData: 0, totalPages: 0 } as unknown as PaginationData<UncategorizedMenuItem[]>;
            } catch (err) {
                handleApiError(err, "Failed to load uncategorized menu", errorNotificationDashboard);
                throw err;
            }
        }
    });
...
export const useSetCategoryMenuMutation = () => {
    ...
    return useMutation({
        ...
        onError: (err) => {
            handleApiError(err, "Failed to assign category", errorNotificationDashboard);
        }
    });
};
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            try {
                ...
                handleApiError(err, "Failed to load products for this category", errorNotificationDashboard);
                throw err;
            }
        },
        enabled: !!categoryId,
