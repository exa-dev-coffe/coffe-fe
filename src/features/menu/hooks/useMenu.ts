...
import {
  ...
  type MenuFormData,
  type MenuItem,
} from "@/features/menu/types/menu.types.ts";
    ...
    queryFn: async () => {
      try {
        ...
        const res = await fetchWithRetry<
          BaseResponse<PaginationData<MenuItem[]>>
        >({
          url,
          method: "get",
        });
...
export const useMenusInfiniteQuery = (
  pageSize: number,
  search?: string,
  categoryId?: number | null,
) => {
      ...
      try {
        ...
        if (res?.data?.success && res.data.data) {
          return {
            ...
            lastPage:
              res.data.data.lastPage ?? res.data.data.data?.length < pageSize,
            pageParam,
          };
        }
