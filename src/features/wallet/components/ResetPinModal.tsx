...
import { HiOutlineKey, HiOutlineMail, HiOutlineCheckCircle } from "react-icons/hi";
import { useChangePinMutation, useSendResetPinCodeMutation, useResetPinMutation } from "@/features/wallet/hooks/useWallet.ts";
...
export const ResetPinModal: React.FC<ResetPinModalProps> = ({ show, onClose }) => {
    ...
    const handleChangePinSubmit = async (e: React.FormEvent) => {
        ...
        if (oldPin.length !== 6 || changeNewPin.length !== 6 || changeConfirmPin.length !== 6) {
    ...
    const handleSendCode = async () => {
        ...
        try {
            ...
            if (err?.response?.status === 429) {
                setResetError("Batas maksimum pengiriman kode verifikasi (3 kali) hari ini telah tercapai.");
            }
        }
    };
    ...
    const handleResetPinSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await resetPinMutation.mutateAsync({ code: otpCode, newPin: resetNewPin });
            handleCloseModal();
        } catch {
            // Error notification is handled in mutation
        }
    ...
    return (
        <Modal show={show} handleClose={handleCloseModal} size="md" title="Transaction PIN Settings">
            <div className="py-2 space-y-5">
                {/* Tabs */}
                <div className="flex bg-slate-100 dark:bg-slate-800/80 p-1 rounded-xl">
                    <button
                        type="button"
                        onClick={() => { setActiveTab("change"); resetForms(); }}
                    ...
                    <button
                        type="button"
                        onClick={() => { setActiveTab("reset"); resetForms(); }}
                        className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-2 cursor-pointer ${
                            activeTab === "reset"
                                ? "bg-white dark:bg-slate-700 text-amber-700 dark:text-amber-400 shadow-sm"
                                : "text-slate-600 dark:text-slate-400 hover:text-slate-900"
                ...
                {activeTab === "change" && (
                    <form onSubmit={handleChangePinSubmit} className="space-y-4">
                        ...
                        <div className="flex gap-3 pt-3">
                            <Button type="button" variant="secondary" size="md" onClick={handleCloseModal} className="flex-1">
                                Cancel
                            </Button>
                            <Button
                                type="submit"
                ...
                {activeTab === "reset" && (
                    <form onSubmit={handleResetPinSubmit} className="space-y-4">
                        ...
                        <div className="p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex flex-col gap-2 text-xs text-amber-900 dark:text-amber-300">
                            <span className="font-semibold">Reset PIN via Verification Code</span>
                            <span>
                                Send a 6-digit OTP code to your registered email. Limited to <strong>max 3 times per day</strong>.
                            </span>
                            <div className="pt-1">
                                <Button
                                    type="button"
                                    ...
                                    disabled={resendTimer > 0 || sendResetPinCodeMutation.isPending}
