...
const DetailMenuSkeleton = () => {
    return (
        <>
            <div
                className={'bg-white animate-pulse flex mt-10 gap-5 p-8 rounded-2xl sm:flex-row flex-col justify-between items-center'}>
                        ...
                        Array.from({length: 5}).map((_, index: number) => {
                            return (
                                <IoStarSharp key={index}
                                             className={'bg-gray-300 animate-pulse w-6 h-6 rounded-full'}></IoStarSharp>
                            );
                        })
                    }
            ...
            <div className={'flex gap-5 md:flex-row flex-col mt-10 w-full'}>
                <div
                    className={'mt-10 shrink-0 md:w-96 w-full bg-white p-8 rounded-2xl flex justify-between items-center'}>
                    <div className={'w-96 h-96 bg-gray-300 mx-auto animate-pulse rounded-2xl'}></div>
                </div>
                <div className={'mt-10 flex flex-col grow bg-white p-8 rounded-2xl'}>
                    ...
                    <div className={'mt-auto ms-auto '}>
                        <div className={'bg-gray-300 animate-pulse w-40 h-10 mt-10 rounded-full'}></div>
                    </div>
                </div>
            </div>
